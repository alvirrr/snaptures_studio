...
@section('title', 'Data Background')

@section('content')
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <h1 class="text-2xl font-bold">Data Background</h1>

        <div class="flex items-center gap-3">
            {{-- Search Form --}}
            <form method="GET" action="{{ route('admin.background') }}" class="flex gap-2">
            ...
            <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                @forelse ($backgrounds as $background)
                    <tr class="hover:bg-blue-50">
                        <td class="px-6 py-4 font-medium">{{ $background->id }}</td>
                        <td class="px-6 py-4">{{ $background->judul }}</td>
                        <td class="px-6 py-4 capitalize">{{ $background->kategori }}</td>
                        <td class="px-6 py-4">
                            <img src="{{ asset($background->gambar) }}" alt="Gambar"
                                class="w-16 h-16 object-cover rounded border">
                        </td>
                        <td class="px-6 py-4 space-x-2">
                            <a href="{{ route('admin.background.edit', $background->id) }}"
                                class="text-blue-600 hover:underline text-sm">Edit</a>
                            <form action="{{ route('admin.background.destroy', $background->id) }}" method="POST"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-4 text-center text-gray-400">Belum ada data background.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
